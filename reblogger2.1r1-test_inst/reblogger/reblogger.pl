...
function displayReblogger(item){
	var source = \"$url\" + item;
	window.open(source,"Reblogger"+item,"width=$cwidth,height=$cheight,status,scrollbars,resizable");
}
EOT
...
EOT
	...
	$comments =~ s/\s+http:\/\/(\S+)/<a target=\'_Blank\' href=\'http:\/\/$1\'>http:\/\/$1<\/a>/gi;
...
# the following substitutions replace the template with the actual comment
	...
	$output =~ s/<\*name\*>/<a href="mailto:$email">$name<\/a>(<a href="http:\/\/$homepage" target="_blank">www<\/a>)/gi;
	...
	# if the user has email notification enable, send an email about this comment to them
		...
		print EMAIL "From: \"Reblogger Email Notification\" <reblogger\@jsoft.ca>\n";
